...
cohen_imgpath = './rsc/covid-chestxray-dataset/images' 
cohen_csvpath = './rsc/covid-chestxray-dataset/metadata.csv'

# path to covid-14 dataset from https://github.com/agchung/Figure1-COVID-chestxray-dataset
fig1_imgpath = './rsc/Figure1-COVID-chestxray-dataset/images'
fig1_csvpath = './rsc/Figure1-COVID-chestxray-dataset/metadata.csv'

# path to https://www.kaggle.com/c/rsna-pneumonia-detection-challenge
rsna_datapath = './rsc/rsna-pneumonia-detection-challenge'
...
# create train and test folders if needed
...
# format as patientid, filename, label, scan type, separated by a space
train_file = open("train_split_v3.txt","a") 
train_file.write('patientid filename label type')
for sample in train:
    if len(sample) == 4:
        info = str(sample[0]) + ' ' + sample[1] + ' ' + sample[2] + ' ' + sample[3].replace(' ', '_') + '\n'
    else:
        info = str(sample[0]) + ' ' + sample[1] + ' ' + sample[2] + ' ' + 'NA' + '\n'
    train_file.write(info)

train_file.close()

test_file = open("test_split_v3.txt", "a")
test_file.write('patientid filename label type')
for sample in test:
    if len(sample) == 4:
        info = str(sample[0]) + ' ' + sample[1] + ' ' + sample[2] + ' ' + sample[3].replace(' ', '_') + '\n'
    else:
